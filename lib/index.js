...
var PlaceholderLoading = (function (rl) {
    var Loading = function Loading(_ref) {
        ...
        var temp = [include, exclude].map(function (v, i) {
            if (isNull(v)) return true;
            var bool = (isArray(v) ? v : [v]).includes(loadKey);
            return i ? !bool : bool;
        }).every(function (v) {
            return v;
        });

        return loading && temp ? !isNull(rl) ? React.createElement(
            'div',
            { className: className },
            null,
            cover ? null : rl,
    ...
    return connect(function (store) {
        var _ref2 = store['@@LOADING'] || {},
            _ref2$key = _ref2.key,
            loadKey = _ref2$key === undefined ? '' : _ref2$key,
...
var requestMiddleware = (function (RE, _ref, store) {
    ...
    return function (next) {
        return function (action) {
            ...
            var requestName = action.request.name;
