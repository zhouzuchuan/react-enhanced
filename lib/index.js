...
var sagas = _interopDefault(require('model-redux/lib/effects/sagas'));
var epics = _interopDefault(require('model-redux/lib/effects/epics'));
...
// loading model 名称
var RE_LOADING_NAME = '_loading_model_';
...
var Loading$1 = reactRedux.connect(function (store) {
...
var Provider2 = function Provider2(props) {
  return React.createElement(reactRedux.Provider, _extends({ store: RE.__store__ }, props));
};
...
var AsyncComponent = (function () {
    ...
    if (isFunction(params)) {
        ...
        loader = component;
        //  提前加载 component
        Promise.all([component].concat(toConsumableArray(isArray(model) ? model : [model])).map(function (v) {
            return v();
        })).then(function (data) {
            var m = data.slice(1);
            m && Object.values(m).forEach(function (v) {
...
var loadingModel = {
    ...
    epics: {
        __SET_LOADING_ACTION__: function __SET_LOADING_ACTION__(epic$) {
            return epic$.pipe(operators.map(function (_ref) {
...
function configureStore() {
    ...
    Object.entries({
        ...
        _reducers: {},
        _models: [],
        asyncReducers: { route: reactRouterRedux.routerReducer },
        asyncSagas: {},
        asyncEpics: {},
    ...
    var _modelRedux$create = modelRedux.create({
        middlewares: [[requestMiddleware.bind(null, RE, {
...
// 获取 immutable 数据
var immutableSelector = function immutableSelector(source, keyArr) {
    return keyArr.reduce(function (r, v) {
