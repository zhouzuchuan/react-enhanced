...
var pull = function pull() {
    ...
    return extractParams(params, function (wh, limit) {
        return pick(wh, isArray$1(limit) ? limit : [limit]);
    });
};
// 存
var push = function push() {
    ...
    return extractParams(params, function (wh, limit) {
        return isObject$1(limit) ? Object.entries(limit).reduce(function (r, _ref3) {
...
var request = function request() {
    ...
    return bindActionCreators(Object.entries(pull.apply(undefined, params)).reduce(function (r, _ref5) {
        ...
        return _extends({}, r, defineProperty({}, k, function (params) {
            ...
            var _ref7 = _extends({}, isFunction$1(options) && { callback: options }, isObject$1(options) && options),
...
var returnArray = function returnArray(v) {
    ...
    return (isArray$1(v) ? v : [v]).some(function (o) {
        return s[o];
    });
};
...
var Loading$1 = function (_React$Component) {
    ...
    createClass(Loading, [{
        ...
        value: function render() {
            ...
            if (update) {
                ...
                return !isNull(RE.requestLoading) ? mask ? React.createElement(
                    'div',
...
Loading$1.propTypes = ['include', 'exclude'].reduce(function (r, v) {
...
var PlaceholderLoading = connect(function (store) {
...
var Install = (function () {
    ...
    return function (WrappedComponent) {
        return function (props) {
            console.warn('Install 将在未来的2.0版本删除，Loading以及AsyncComponent的使用，请查看文档 https://zhouzuchuan.github.io/react-enhanced/#/base/hoc');
            var newProps = _extends({}, props, isString(inject) || isArray$1(inject) ? pick(RE, inject) : {});
...
var deal = function deal(l, p) {
    return isFunction$1(l) ? fn(p) : pick(p, isArray$1(l) ? l : [l]);
};
...
var Model = (function (modelFn) {
    return function (WrappedComponent) {
        return function (props) {
            ...
            var newProps = (isArray$1(modelFn) ? modelFn : [modelFn]).reduce(function (r, v) {
                return _extends({}, r, isFunction$1(v) ? v({ pull: pull$$1, push: push$$1, dispatch: dispatch, request: request$$1, getState: getState }, props) || {} : {});
            }, {});
    ...
    return function (next) {
        return function (action) {
            ...
            return request().then(function (result) {
                ...
                var limitData = isString(resultLimit) ? take(transferData, resultLimit) : isArray$1(resultLimit) ? resultLimit.reduce(function (r, v) {
                    if (!isString(v)) {
...
var observableMiddlevare = (function (RE, store) {
    ...
    return function (next) {
        return function (action) {
            ...
            if (isFunction$1(__RE_OBSERVABLE_RESOLVE__) && isFunction$1(__RE_OBSERVABLE_REJECT__)) {
...
function registerModel(sagaMiddleware, epicMiddleware, models) {
    var deal = (isFunction$1(models) ? [models(pick(RE, ['pull', 'push', 'request']))] : Array.isArray(models) ? models : [models]).filter(function (model) {
        if (!isObject$1(model)) {
...
var AsyncComponent = (function () {
    ...
    if (isMore) {
        ...
        return Loadable.Map(_extends({}, defaultParams, rest, {
            loader: (model ? isArray$1(model) ? model : [model] : []).reduce(function (r, v, i) {
...
function configureStore() {
    ...
    // 注入默认model
    [].concat(toConsumableArray(isArray$1(models) ? models : [models]), [loadingModel]).forEach(function (v) {
